...
def build_action_prediction_dataset(args):
    ...
    for example in next_example(playthroughs):
        ...
        current_graphs = [graph_dataset.compress(candidate["graph_{}".format(args.graph_type)])
                          for candidate in candidates]

        # For each data point we want the following 6 keys:
        # game, step, previous_graph, target_action, current_graph, action_choices
        for i, candidate in enumerate(candidates):
            dataset.append({
                ...
                "target_action": action_choices[i],
...
def main():
    ...
    build_action_prediction_dataset(args)
