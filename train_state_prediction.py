...
from state_prediction_dataset import SPData
    ...
    try:
        while(True):
            if episode_no > agent.max_episode:
                break
            agent.train()
            target_graph, previous_graph, action, graph_choices = env.get_batch()
            curr_batch_size = len(target_graph)

            loss, _, _, _ = agent.get_state_prediction_logits(previous_graph, action, target_graph, graph_choices)
            # Update Model
            ...
            if episode_no % agent.report_frequency <= (episode_no - curr_batch_size) % agent.report_frequency:
                if agent.run_eval:
                    eval_loss, eval_acc = evaluate.evaluate_state_prediction(env, agent, "valid")
